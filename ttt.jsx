  ...
  return (
    <div className="game-board">
      ...
      <div id="info">
        ...
        <h1>{status}</h1>
       {/* <button onClick={(e) => {setGameState([])}}>Clear</button> */}
      </div>
    </div>
  );
};
...
const Square = ({ takeTurn, id, player }) => {
  ...
  const [color, setColor] = React.useState('rgb(255, 198, 255)');  
  const [col, setCol] = React.useState("white");
    
  const palet =['rgb(253, 255, 182)', 'rgb(202, 255, 191)', 'rgb(155, 246, 255)', 'rgb(189, 178, 255)']
  const getRandomColor = () => palet[Math.floor(Math.random()*4)]

  const setter = (p) => (p == 1) ? setCol('red') : setCol('black');
  ...
  return (
    <button
      ...
      onClick={(e) => {
        setTik(takeTurn(id));
        setFilled(true);
        console.log(`Square: ${id} filled by tik: ${tik} and player ${player}`);
